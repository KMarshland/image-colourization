...
def init_file_path(directory):
    ...
    for file_name in os.listdir(directory):
        ...
        print('\t' + file_path)
        if imghdr.what(file_path) is not 'png' and imghdr.what(file_path) is not 'jpeg':
            print('\tNot an image, skipping. Type: %s' % imghdr.what(file_path))
            continue
        if not debug:
